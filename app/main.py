    ...
    with db:
        with db.cursor() as cursor:
            cursor.execute(
                ...
                (userid, logday, water, sleeptime, steps),
            )
...
def useractivity(userid=0):
    ...
    with get_db() as db:
        with db.cursor() as cursor:
            cursor.execute(
                "SELECT userid, logday, water, sleeptime, steps FROM activities WHERE userid = %s;",
                (userid,),
            )
...
def useractivityjson(userid=0):
    with get_db() as db:
        with db.cursor() as cursor:
            ...
            activities = cursor.fetchall()
    return jsonify(
        {"user": userid, "activities": activities, "prediction": predict_water()}
    )
